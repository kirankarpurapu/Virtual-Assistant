...
# aws resources
#bucket = connection_s3.get_bucket('recognito-s3')
...
def compare_face_encodings(known_faces, face):
    ...
    return matches
#url is the second last parameter
def write_to_dynamodb(user_id, photo_id, contact_id, image_name, encodings):
        print("****************************")
...
def write_to_database(image_info, image_name, encodings, u_id):
        print("*********************************")
...
#url is the second last parameter
        dynamodb_response = write_to_dynamodb(user_id, u_id, image_info, image_name, encodings)
        return dynamodb_response

def get_matching_photo_details(image, user_id):
        # get all the rows from dynamo db
        images = table.scan()
        # images = table.get_item(USER_ID = user_id)

        encodings_array = []
        info_array = []
        for item in images:
                print("**********************")
                # print("userid {0}, {1}").format(user_id, item['USER_ID'])
        ...
        if 'contactid' not in data:
                ...
                return json_result
                        ...
                        if len(encodings) == 1:
                                ...
                                write_to_database(image_info, image_name, encodings, user_identity)
...
def test_image():
        if request.method == 'POST':
                ...
                if 'base64' not in data:
                        return 'insufficient data in the post request, missing /"base64/" field'
                base64_data = data['base64']
                if 'userid' not in data:
                    print("missing userid")
                    return "userid not present, insufficient data"
                user_identity = data['userid']
                print("user id ", user_identity)
                image_data = base64.b64decode(base64_data)
                test_file = write_to_file("test.jpg", image_data)
                return_value_json = get_matching_photo_details("test.jpg", user_identity)
